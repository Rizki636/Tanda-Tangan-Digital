...
assets/style.css
assets/qrcode.min.js  → library pembuat kode QR, di-hosting sendiri (bukan CDN luar)
```

**Catatan soal library eksternal**: `sign.html` masih memuat `pdf-lib` dan `pdf.js` dari CDN publik
(`unpkg.com` dan `cdnjs.cloudflare.com`) untuk membaca/menulis PDF. Kalau suatu saat CDN itu
juga bermasalah di jaringan Anda (muncul error semacam "PDFLib is not defined" atau
"pdfjsLib is not defined"), beri tahu saya — bisa saya bundel juga jadi file lokal seperti
yang sudah dilakukan untuk `qrcode.min.js`.
...
- **Tampilan hasil scan QR dibuat seminimal mungkin.** Begitu QR dipindai, yang
  langsung tampil hanya kartu "Tercatat Sah" (nama, judul, waktu, status, hash) —
  menu navigasi dan kotak input ID/cek keutuhan disembunyikan secara otomatis.
  Ada tombol "🔍 Periksa ID lain / cek keutuhan dokumen" di bawah kartu itu bagi
  yang mau memeriksa lebih lanjut. Kalau `verify.html` dibuka langsung (bukan
  lewat QR, tanpa parameter `?id=`), tampilan lengkapnya muncul seperti biasa.
